...
International Typographic Style. One grotesque (Archivo), flush-left ragged-right type, a 2px rule and running head on every slide, warm-white paper and one red. A single red square morphs through the deck: a 400px block on the cover, a 16px dot beside the page number, a bar on section dividers, full-bleed on the closing slide.
...
Mid-century product design after Dieter Rams and Braun. Warm grey ground, charcoal text, hairlines, a geometric sans (Jost) and one orange indicator, the only colour on the page. The indicator orbits a dial on the cover, then morphs through the deck as a footer dot, a disc on the black section dividers and a large disc at the end.
...
Every layout carries speaker notes on its demo slide explaining what it is for and which Bento feature it uses (morph, step reveal, count-up, state slide, ken-burns, dash-march).
...
2. Give every slide `notes` and every element an `id`. Shared ids across consecutive slides are what make morph transitions work.
...
Design constraints the existing themes follow: one accent colour, one or two typefaces, 96px side margins, body text 22px or larger, nothing under 14px.
...
- Step reveals (`fx.step`) consume arrow presses. The vendored `render_check.mjs` presses through them before capturing each page; the upstream copy in the bento-slides skill does not and stops short of the last slides.
