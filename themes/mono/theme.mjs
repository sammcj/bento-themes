// Mono: a well-set terminal. Near-black ground, warm off-white text, one
// monospace face (JetBrains Mono) and a single phosphor green used for the
// prompt glyph, the block cursor and the chart's first series. No scanlines,
// glow or animation: the ids stay stable so a morph can be switched on later.
// The recurring mark is a block cursor ("mark") sized 0.6em x 1.1em of the
// text beside it, and a ">" prompt glyph ("prompt") in the accent.
...
const t = { fontFamily: FONT, color: TEXT, accent: GREEN };
const f = factory(t);
...
const chrome = ({ inverted = false, mark = true } = {}) => {
  ...
  return [
    ...
    ...(mark ? [cursor(16, { x: 1174, y: 662, fill: inverted ? GROUND : GREEN })] : []),
  ];
};
...
const numbers = (demo) => {
  ...
  const els = stats.flatMap(([v, l], i) => {
    const x = COLS[3].x[i];
    return [
      f.text({ id: `st-v${i}`, ...ph(demo, v, v), fontSize: 120, fontWeight: 700, lineHeight: 1.0, color: i === 1 ? GREEN : TEXT, x, y: 244, w: COLS[3].w, h: 128 }),
...
const chart = (demo) => ({
  ...
  background: GROUND,
  transition: "none",
  notes: "Chart. Series in phosphor, off-white and grey; axes in the hairline colour. Bar and line data are plain numbers; only pie takes {name, value}. The legend must be an object to render. textStyle.fontFamily is set so the chart uses the mono face rather than the browser default.",
  elements: [
    ...chrome(),
...
const table = (demo) => ({
  ...
  background: GROUND,
  transition: "none",
  notes: "Comparison table. Header row on a lighter charcoal, 1px hairlines, no zebra. Tables are for specs and comparisons; a numeric trend belongs in the chart layout.",
  elements: [
    ...chrome(),
...
const process = (demo) => {
  ...
  return {
    ...
    background: GROUND,
    transition: "none",
    notes: "Process. Four numbered square nodes on a dotted phosphor track. In the demo, step 2 is clickable: a transparent rect over it links to a state slide, and the left arrow returns.",
    elements: [
      ...chrome(),
...
const code = (demo) => ({
  ...
  background: GROUND,
  transition: "none",
  notes: "Code. A code element on a charcoal panel with a one-line caption. Set grammarName to the language (py, sh, js, ts, go, rust, sql, json, yaml, diff). Twelve lines at 22px fill the panel; long lines do not wrap, so keep them under 78 characters. Token colours come from theme.codePalette.",
  elements: [
    ...chrome(),
...
const image = (demo) => ({
  ...
  background: GROUND,
  transition: "none",
  notes: "Full-bleed image with a dark scrim and the caption at the bottom left. Replace the placeholder asset with a screenshot or photo downscaled to 2560px before embedding. Keep captions in the text element, since text baked into an image cannot be edited.",
  elements: [
...
const placeholderSvg = () => {
  ...
  const svg =
    ...
    `<g fill="${GREEN}" opacity="0.35">${dots.join("")}</g>` +
    `<path d="${wave.join(" ")}" fill="none" stroke="${GREEN}" stroke-width="2"/></svg>`;
...
export default function makeDoc({ root }) {
  ...
  const layouts = builders.map((b) => {
    const s = b(false);
    return { id: `layout-mono-${s.id.slice(2)}`, name: s.name, background: s.background, transition: s.transition, notes: "", elements: s.elements };
  });
  ...
  return {
    ...
    meta: { author: "", company: "Company", subject: "", event: "", keywords: "mono, terminal, monospace, dark, engineering" },
    theme: {
      background: GROUND,
      color: TEXT,
      accent: GREEN,
      fontFamily: FONT,
      headingFamily: FONT,
      palette: { bg2: PANEL, tx2: GREY, accent2: TEXT, accent3: GREY, accent4: HAIR, hlink: GREEN },
      chartPalette: [GREEN, TEXT, GREY],
      codePalette: { k: GREEN, f: TEXT, s: STRING, n: TEXT, c: GREY, p: GREY, a: GREEN, d: GREY },
      table: TABLE_STYLE,
    },
